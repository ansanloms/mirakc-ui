...
視聴画面の右パネル「実況コメント」タブに、視聴中チャンネルの実況コメントを時系列リスト表示する ([#38](https://github.com/ansanloms/mirakc-ui/issues/38))。コメント取得元 (ニコ生 / NX-Jikkyo) はプラッガブルで、複数を同時購読してまとめて表示する。取得元の有効/無効は設定画面で行い、視聴側にフィルタは持たない。
...
- SSE の `sources` イベント = サーバが購読できた取得元。`use-live-comments` が受信したコメントを**フィルタせず全件**蓄積する。**視聴側に取得元の ON/OFF フィルタは持たない** — 取得元の有効/無効は設定画面 (`/settings/live-comments` の有効スイッチ) が唯一のソース。視聴側に二重で持たない設計。
- 取得元が**複数あるとき各コメントに取得元バッジ** (色・タグは `client/lib/comment-source.ts`、色は code・ラベル/タグは locales `liveComment.source.*`)。`sources.length > 1` で出し分ける。
- バッジは実況タブ (`LiveCommentTab` → `CommentFeed`) とプレイヤーの映像上オーバーレイ (`Player` → `CommentFeed onVideo`) の両方に出る。
