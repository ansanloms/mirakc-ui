...
- 保存先は Deno KV `["settings", "live-comment"]` (`server/store/live-comment-settings.ts`、API は `server/routes/live-comment-settings.ts` の GET/PUT `/api/live-comment-settings`)。GET は未保存時に組み込み対照表からの既定値 + 自動補完候補 (`suggestions`) を返す。
...
- 検証 (形式・serviceId / チャンネル ID の重複) は純粋共有 `server/lib/live-comment-settings.ts`。**有効行のみ厳密検証**し、serviceId 重複だけ有効・無効問わず禁止。client のフォームと server の PUT が同一ロジックを使う。
