...
`server/routes/mirakc.ts` が mirakc バックエンドへのプロキシとして機能する。環境変数 `MIRAKC_API_URL` でバックエンド URL を指定する。`server/main.ts` で `/api/mirakc` にマウント。CORS 回避のためサーバーサイドプロキシパターンを採用。
