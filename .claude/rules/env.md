...
- `TZ` — 日付表示のタイムゾーン（IANA タイムゾーン ID、例: `Asia/Tokyo`）。標準の `TZ` 環境変数で、`server/main.ts` が `Temporal.Now.timeZoneId()` で 1 度だけ解決し、(1) 録画通知・録画ファイル名の整形（`notifyProgramEvent` / `startKeywordRecordingJob` に注入）と (2) `GET /api/config` の `timeZone` に使う。クライアントは起動時に `/api/config` を取得して `setTimeZone()` で全体の日時表示をこの値に揃える（`client/lib/datetime.ts`）。未設定なら実行環境の既定（**Docker コンテナでは UTC**）。通知や UI の時刻が UTC になる場合はこれを設定する。`.env`（ホスト実行・`--env-file`）／`.devcontainer/.env`／本番コンテナの環境変数（compose の `environment:` や `docker run -e TZ=...`）で渡す。
